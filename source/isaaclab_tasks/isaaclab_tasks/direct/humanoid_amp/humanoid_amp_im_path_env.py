...
class HumanoidAmpEnv(DirectRLEnv):
    ...
    def set_test_ang_speed(self, fwd_ang_speed: float | None):
        """If speed is not None, all envs will use this fixed speed at reset."""
        if fwd_ang_speed is None:
            self.test_ang_speed = None
        else:
            self.test_ang_speed = float(fwd_ang_speed)
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
            ...
            if self.demo_mode and (self.test_fwd_speed is not None):
                ...
                # --- Curriculum: record finished episodes & update yaw range ---
                if self.cfg.second_training:
                    valid = self._yaw_reward_steps[env_ids] > 0
                    if valid.any():
                        ep_means = (self._yaw_reward_sum[env_ids[valid]]
                                    / self._yaw_reward_steps[env_ids[valid]])
                        ...
                        if self._curriculum_ep_count >= self._curriculum_N:
                            ...
                            if (buf_mean > self._curriculum_threshold
                                    and self._curriculum_ang_half_range < self._curriculum_max_half_range):
                                old_r = self._curriculum_ang_half_range
                                self._curriculum_ang_half_range = min(
                ...
                self.desired_ang_speeds[env_ids] = reference_ang_speed
...
# --- PREVIOUS VERSION (quat_mul based — caused JIT issues) ---
# @torch.jit.script
# def compute_amp_obs_yaw_invariant(...):
...
def compute_amp_obs_yaw_invariant(
    ...
    """Yaw-invariant AMP observations (v2 — explicit 2D rotation, no quat_mul).

    Computes world-frame tangent/normal and velocities using the proven
    quaternion_to_tangent_and_normal helper, then applies a simple 2D
    rotation by -yaw around Z using cos/sin.  The yaw angular-velocity
    component is zeroed so the discriminator is fully heading-blind.
    """
    # --- 1. Extract yaw from root quaternion (w, x, y, z) ---
    w = root_rotations[:, 0]
    x = root_rotations[:, 1]
    y = root_rotations[:, 2]
    z = root_rotations[:, 3]
    ...
    # --- 2. Tangent + normal: compute in world frame, then rotate by -yaw ---
    tn_w = quaternion_to_tangent_and_normal(root_rotations)      # [N, 6]
    tx = tn_w[:, 0]; ty = tn_w[:, 1]; tz = tn_w[:, 2]
    nx = tn_w[:, 3]; ny = tn_w[:, 4]; nz = tn_w[:, 5]
    tangent_normal_local = torch.stack([
         c * tx + s * ty, -s * tx + c * ty, tz,
         c * nx + s * ny, -s * nx + c * ny, nz,
    ], dim=1)                                                    # [N, 6]

    # --- 3. Rotate linear velocity by -yaw ---
    vx = root_linear_velocities[:, 0]
    vy = root_linear_velocities[:, 1]
    vz = root_linear_velocities[:, 2]
    lin_vel_local = torch.stack([
         c * vx + s * vy,
        -s * vx + c * vy,
        vz,
    ], dim=1)                                                    # [N, 3]

    # --- 4. Rotate angular velocity by -yaw and ZERO the yaw-rate ---
    #   Yaw-rate (Z component) is invariant under Z-rotation, so the
    #   discriminator would still see it.  Zeroing it removes the last
    #   heading-dependent signal from the AMP observation.
    ax = root_angular_velocities[:, 0]
    ay = root_angular_velocities[:, 1]
    ang_vel_local = torch.stack([
         c * ax + s * ay,
        -s * ax + c * ay,
        torch.zeros_like(ax),
    ], dim=1)                                                    # [N, 3]

    # --- 5. Rotate key-body relative positions by -yaw ---
    rel = key_body_positions - root_positions.unsqueeze(-2)      # [N, K, 3]
    rx = rel[:, :, 0]           # [N, K]
    ry = rel[:, :, 1]
    rz = rel[:, :, 2]
    cu = c.unsqueeze(1)         # [N, 1]  — broadcasts over K
    su = s.unsqueeze(1)
    ...
    rel_local_flat = rel_local.reshape(rel_local.shape[0], -1)  # [N, K*3]

    return torch.cat((
        dof_positions,
        dof_velocities,
        root_positions[:, 2:3],
        tangent_normal_local,
        lin_vel_local,
        ang_vel_local,
        rel_local_flat,
    ), dim=1)