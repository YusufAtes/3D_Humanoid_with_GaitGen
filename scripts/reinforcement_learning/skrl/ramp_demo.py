...
parser.add_argument("--test_angle", type=float, default=0.0, help="Ramp slope angle in degrees for the test.")
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, experiment_cfg: dict):
    ...
    test_angle = args_cli.test_angle
    env_cfg.test_slope_deg = test_angle
    ...
    for desired_speed in speed_schedule:
        ...
        while True:
            ...
            if demo_step == 148:
                ...
                print(f"Ramp Angle: {test_angle} ,Desired Speed: {desired_speed}, Actual Speed: {avg_speed}, Succes: {True}")
                print("------------------------------------------------------------")
            ...
            if done == True:
                avg_speed = wrapped_env.unwrapped.avg_speed
                print(f"Ramp Angle: {test_angle} ,Desired Speed: {desired_speed}, Actual Speed: {avg_speed}, Succes: {False}")
                print("------------------------------------------------------------")
                episode_current_vel.append(avg_speed)
                # Store results for CSV export
                results_data.append({
    ...
    print(f"Time taken for angle {test_angle} is: {time.time()-t0} seconds")
    # Save results to CSV file (append mode — write header only if file is new)
    csv_file_path = os.path.join(log_dir, "ramp_demo_results.csv")
    file_exists = os.path.isfile(csv_file_path)
    with open(csv_file_path, 'a', newline='') as csvfile:
        fieldnames = ['desired_speed', 'actual_speed', 'reward', 'test_angle', 'success']
